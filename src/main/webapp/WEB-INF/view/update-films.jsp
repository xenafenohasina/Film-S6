...
<%
  V_Films films = (V_Films) request.getAttribute("films");
  List<Statut> statuts = (List<Statut>)request.getAttribute("status");
...
<div class="container">
  ...
  <div class="cs-prifile_wrap">
    ...
    <div class="cs-profile_right">
      ...
      <h2 class="cs-section_heading cs-style1">Modifier un film</h2>
      <div class="cs-height_25 cs-height_lg_25"></div>
      <form class="row" action="<%= request.getContextPath() %>/modifier-films" method="post">
        <input type="hidden" name="idFilms" value="<%=films.getIdFilms() %>">
        <div class="row">
          <div class="col-lg-6">
            <div>
              Nom: <input type="text" name="nom" class="cs-form_field cs-white_bg" value="<%=films.getNom() %>">
            </div>
            <div class="cs-height_25 cs-height_lg_25"></div>
            <div>
              Date De Création: <input type="date" class="cs-form_field cs-white_bg" name="dateCreation" value=<%=films.getDateCreation() %> >
            </div>
              ...
              <div>
                ...
                <p>
                    <% for(Statut s : statuts){ %>
                      <% if( s.getIdStatut() == films.getIdStatut() ){ %>
                        <input type="radio" name="idStatut" checked="true" value="<%=films.getIdStatut() %>"> <%=films.getStatut() %>
                      <% }else{ %>
                        <input type="radio" name="idStatut" value="<%=s.getIdStatut() %>"> <%=s.getNom() %>
                      <% } %>
                    <% } %>
                </p>
