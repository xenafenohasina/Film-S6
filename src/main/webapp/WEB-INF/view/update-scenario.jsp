...
<%
    V_Scenario scenario = (V_Scenario) request.getAttribute("scenario");
    Temps [] temps = (Temps [])request.getAttribute("temps");
    List<Personnage> personnages = (List<Personnage>) request.getAttribute("personnages");
    List<V_Dialogue> dialogues = null;
    List<V_Action> actions = null;
    if( scenario.getIdtype() == 1){
        actions = (List<V_Action>) request.getAttribute("listes");
    }
    else{
        dialogues = (List<V_Dialogue>) request.getAttribute("listes");
    }
...
<div class="container">
    ...
    <div class="cs-prifile_wrap">
        ...
        <div class="cs-profile_right">
            ...
            <h2 class="cs-section_heading cs-style1">Modifier un Scénario</h2>
            <div class="cs-height_25 cs-height_lg_25"></div>
            <% if(scenario.getIdtype() == 1 ){ %>
                <form class="row" action="<%= request.getContextPath() %>/update-action" method="post">
                    <input type="hidden" name="idScenario" value="<%=scenario.getIdScenario() %>">
                    <div class="row">
                        <div class="col-lg-6">
                            <div>
                                Type: <%=scenario.getNomtype() %>
                            </div>
                            <div class="cs-height_25 cs-height_lg_25"></div>
                            <% if( user.getIdrole() != 1){ %>
                                <div>
                                ...
                                <div class="cs-height_25 cs-height_lg_25"></div>
                                <div>
                                    Tournage: <%=scenario.getTemps() %>
                                </div>
                                <div class="cs-height_25 cs-height_lg_25"></div>
                            <% }else{ %>
                            <div>
                                <p>Duree: <input type="time" step="1" name="duree" value=<%=scenario.getDuree() %> ></p>
                            </div>
                            <div>
                                <p>Tournage:
                                    <% for(Temps t : temps){ %>
                                        <% if( t.getIdTemps() == scenario.getTempstravail()){ %>
                                            <input type="radio" name="tempsTravail" checked="true" value="<%= scenario.getTempstravail() %>"><%= scenario.getTemps() %>
                                        <% }else{ %>
                                        <input type="radio" name="tempsTravail" value="<%=t.getIdTemps() %>"><%= t.getTemps() %>
                                        <% } %>
                                    <% } %>
                                </p>
                            </div>
                            ...
                            <% for(V_Action action : actions){ %>
                                <input type="hidden" name="idAction" value="<%=action.getIdAction() %>">
                                <div class="row">
                                    <div class="col-lg-4">
                                        <select name="idPersonnage" class="cs-form_field cs-white_bg">
                                            <option value="<%=action.getIdPersonnage() %>"><%=action.getNom() %></option>
                                            <% for(Personnage p : personnages){ %>
                                            <option value="<%=p.getIdPersonnage() %>"><%=p.getNom() %></option>
                                            <% } %>
                                        </select>
                                    </div>
                                    <div class="col-lg-4">
                                        <textarea class="cs-form_field cs-white_bg" name="descriptions"><%= action.getDescriptions()%></textarea>
                                    </div>
                                    <div class="cs-height_25 cs-height_lg_25"></div>
                                </div>
                            <% } %>
            ...
            <% }else{ %>
            <form class="row" action="<%= request.getContextPath() %>/update-dialogue" method="post">
                <input type="hidden" name="idScenario" value="<%=scenario.getIdScenario() %>">
                <div class="row">
                    <div class="col-lg-6">
                        <div>
                        ...
                        <% for(V_Dialogue dialogue : dialogues){ %>
                            <input type="hidden" name="idDialogue" value="<%=dialogue.getIdDialogue() %>">
                            <div class="row">
                                <div class="col-lg-4">
                                    <select name="idPersonnage" class="cs-form_field cs-white_bg">
                                        <option value="<%=dialogue.getIdPersonnage() %>"><%=dialogue.getNom() %></option>
                                        <% for(Personnage p : personnages){ %>
                                        <option value="<%=p.getIdPersonnage() %>"><%=p.getNom() %></option>
                                        <% } %>
                                    </select>
                                ...
                                <div class="col-lg-4">
                                    <textarea class="cs-form_field cs-white_bg" name="script" cols="8" row="5"><%= dialogue.getScript() %></textarea>
                                </div>
                                <div class="col-lg-4">
                                    <input class="cs-form_field cs-white_bg" type="text" name="emotion" value="<%= dialogue.getEmotion() %>" >
                                </div>
                                <div class="cs-height_25 cs-height_lg_25"></div>
                            </div>
                        <% } %>
